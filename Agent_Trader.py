...
class Strategy_model(Strategy_Config):
    def __init__(self, portfolio_name, strategy_name,experiment_name):
        ...
        self.virtual_GPU =get_virtual_GPU(self.GPU_idx,self.GPU_mem)

    def get_next_day_action(self,DateI,df_account, i_eb,mumber_of_stock_could_buy,l_holding, L_Eval_Profit_low_flag):
        with tf.device(self.virtual_GPU):
...
class Strategy_agent(Strategy_agent_base,Strategy_model,Strategy_agent_Report):
    def __init__(self, portfolio_name, strategy_name,experiment_name):
        Strategy_agent_base.__init__(self,portfolio_name, strategy_name,experiment_name)
        Strategy_model.__init__(self,portfolio_name, strategy_name,experiment_name)
    ...
    def remove_old_account(self):
        if os.path.exists(self.iFH.AT_account_dir):
            shutil.rmtree(self.iFH.AT_account_dir)
        os.mkdir(self.iFH.AT_account_dir)
    def start_strategy(self,DateI):
        self.remove_old_account()
        ...
        l_a,l_ADlog=self.get_next_day_action(DateI, df_account, self.i_eb,mumber_of_stock_could_buy, l_holding, L_Eval_Profit_low_flag)
