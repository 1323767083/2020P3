...
class env_reward_basic_old:
    def __init__(self,scaler_factor,reward_type,lgc):
        self.lgc=lgc
        assert hasattr(self.lgc.specific_param,"flag_reward_punish_no_action"),"flag_reward_punish_no_action is a mandatory param"
        if self.lgc.specific_param.flag_reward_punish_no_action:
            self.Sucess_buy = 0.0
            self.Fail_buy   = -0.0001
            self.Fail_sell  = -0.0001
            self.No_action  = -0.0001
            self.Tinpai     = -0.0001
        else:
            self.Sucess_buy = 0.0
            self.Fail_buy = 0.0
        ...
        supported_reward_type=["raw","scaler_clip_shift","clip_shift"]
        if reward_type in supported_reward_type:
            self.Success_sell = getattr(self, reward_type)
        else:
            assert False, reward_type

    def scaler_clip_shift(self,profit):
        raw_profit=(profit-self.lgc.specific_param.RewardShift) * self.scale_factor
        adj_profit=1 if raw_profit > 1 else -1 if raw_profit < -1 else raw_profit
        return adj_profit

    def clip_shift(self,profit):
        return 1 if profit > self.lgc.specific_param.RewardShift else -1
