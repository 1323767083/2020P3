...
class Simulator_LHPP2V7(Simulator_LHPP2V5):
    def __init__(self, data_name, stock,called_by):
        Simulator_LHPP2V5.__init__(self, data_name, stock,called_by)
...
class Simulator_LHPP2V8:
    def __init__(self, data_name, stock,called_by):
        assert not lc.flag_multi_buy
        assert lc.P2_current_phase == "Train_Buy"
        self.called_by = called_by
        self.data_name = data_name
        #self.i_PSS_V8=Phase_State_V8(called_by)
    ...
    def step(self,action):
        action=self.i_PSS.check_need_force_state(action)
        action_str=lc.action_type_dict[action]
        return_message,reward,state, support_view_dic=self.step_comm(action_str)
        support_view_dic["action_return_message"]=return_message
        Done_flag=self.i_PSS.update_phase_state(support_view_dic,action,return_message)
        self.i_PSS.fabricate_av_and_update_support_view(state, support_view_dic)
        return state, reward,Done_flag,support_view_dic
