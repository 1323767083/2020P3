...
class ana_reward_data(are_esi_reader):
    ...
    def _get_are_summary_1ET(self, evalT):
        ...
        if os.path.exists(ET_summary_fnwp) and os.path.exists(effective_count_fnwp) and os.path.exists(stock_statistic_fnwp):
            try:
    ...
    def get_are_summary(self):
        ...
        for ET_idx, EvalT in enumerate(self.LEvalT):
            ...
            if not flag_opt:
                ...
                if ET_idx==0:
                    df = dfi
                    dfse = pd.DataFrame([Dic_Summary.values()], columns=Dic_Summary.keys())
                else:
                    df = df.append(dfi, ignore_index=True)
                    dfse = dfse.append(pd.DataFrame([Dic_Summary.values()], columns=Dic_Summary.keys()),ignore_index=True)
