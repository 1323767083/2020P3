...
class record_variable:
    def __init__(self, dirwp, checked_name="loss", checked_threadhold=10):
        ...
        self.checked_threadhold = checked_threadhold
...
class record_variable2(record_variable):
    ...
    def saver(self):
        if self.F_need_record and self.F_trainer_recorded and self.F_brain_recorded and self.F_process_recorded:
            ...
            fn = "SC{0}_CC{1}.pkl".format(compressed_inputs[2]["saved_train_count"],compressed_inputs[2]["current_train_count"])
            fnwp = os.path.join(self.dirwp, fn)
            with open(fnwp, 'w') as f:  # Python 3: open(..., 'wb')
...
class record_variable3(record_variable2):
    def saver(self):
        if self.F_need_record and self.F_trainer_recorded and self.F_brain_recorded and self.F_process_recorded:
            compressed_inputs = self._compress_content([self.RD_trainer, self.RD_brain, self.RD_process])
            if not hasattr(self, "current_stc"):
                self.current_stc=compressed_inputs[2]["saved_train_count"]
                self.current_dir=os.path.join(self.dirwp,"SC{0}".format(self.current_stc))
                if not os.path.exists(self.current_dir):
                    os.mkdir(self.current_dir)
            elif self.current_stc!=compressed_inputs[2]["saved_train_count"]:
                self.current_stc=compressed_inputs[2]["saved_train_count"]
                self.current_dir=os.path.join(self.dirwp,"SC{0}".format(self.current_stc))
                if not os.path.exists(self.current_dir):
                    os.mkdir(self.current_dir)
            ...
            fn = "CC{0}.pikle".format(compressed_inputs[2]["current_train_count"])
            fnwp = os.path.join(self.current_dir, fn)
            with open(fnwp, 'w') as f:  # Python 3: open(..., 'wb')
                pickle.dump(compressed_inputs, f)
            self.F_need_record = False
...
class record_variable2_reader:
    def __init__(self,data_name,system_name, CLN_env_read_data):
        self.data_name=data_name
        self.system_name=system_name
        self.CLN_env_read_data=CLN_env_read_data
        self.class_env_read_data=globals()[self.CLN_env_read_data]
        self.dirwp=os.path.join(sc.base_dir_RL_system,self.system_name,"record_state")
        self.dic_dh_index={}
        self.l_sdh=[]
...
class record_variable3_reader(record_variable2_reader):
    def __init__(self,data_name,system_name, CLN_env_read_data):
        record_variable2_reader.__init__(self,data_name,system_name, CLN_env_read_data)
    def _get_fnwp(self,sc, cc):
        current_dir = os.path.join(self.dirwp, "SC{0}".format(sc))
        fn = "CC{0}.pikle".format(cc)
        fnwp = os.path.join(current_dir, fn)
        return fnwp
...
class get_recorder_OS_losses:
    def __init__(self,system_name):
        ...
        #self.ir = record_variable2_reader(self.lgc.data_name, system_name, self.lgc.CLN_env_read_data)
        self.ir = globals()["{0}_reader".format(self.lgc.CLN_record_variable)](self.lgc.data_name, system_name,
                                                                       self.lgc.CLN_env_read_data)
        '''
    ...
    def get_losses_per_stc(self, stc):
        ...
        for tci in range(self.lgc.num_train_to_save_model):
            ...
            record = [stc, tc,count_valid_record, count_valid_record_sell, count_invalid_record] + raw_loss[1]+[npr.mean()]
...
class record_analysis:
    ...
    def __init__(self, system_name):
        ...
        self.i_state = globals()["{0}_reader".format(lc.CLN_record_variable)]("T5", system_name, lc.CLN_env_read_data)
