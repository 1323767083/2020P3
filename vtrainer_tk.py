...
class vtrainer_tk(tk.Frame):
    def __init__(self, parent, param):
        ...
        self.i_train_data_surf = train_data_surf(self.fig, self.canvas)
    ...
    def init_control_frame(self):
        ...
        #self.train_data_surf_control=self.i_train_data_surf.init_frame__train_data_surf(container,param )
        self.i_train_data_surf.init_frame__train_data_surf(container, param)
    ...
    def optimize_step(self,dc, Dmodel):
        def optimize_step_in(Event):
            i_rd = globals()[lgc.CLN_record_variable](lgc)
...
class train_data_surf:
    def __init__(self, fig, canvas):
        self.fig=fig
        self.canvas=canvas
        self.i_rd = globals()[lgc.CLN_record_variable](lgc)

        self.td_current_sc = -1
        self.td_tc = -1

    def init_frame__train_data_surf(self, container,param):
        width_left = 15
        width_right = 20
        current_row = 0
        tk.Label(container, text=param["title"], width=width_left+width_right, anchor=tk.CENTER,pady = 10 ).\
        ...
        control["B_left"].bind("<Button-1>", func=self.left_step(control))
        control["B_right"].bind("<Button-1>", func=self.right_step(control))
        control["B_update"].bind("<Button-1>", func=self.train_data_update(control))
    ...
    def show_train_datas(self, fig,saved_tc, tc, selected_half):
        ...
        if self.td_current_sc!=saved_tc or self.td_tc!=tc:
            ...
            for idx in range(len(RD_trainer[0])):
                ...
                record = [rs["stock"], rs["date"], rs["action_taken"], rs["action_return_message"], rr, rs["old_ap"]]
                self.show_records.append(record)
            #self.show_records = [['%.2f' % float(j) for j in i] for i in self.show_records]

        if len(self.show_records)<=125:
            base_point = 0
        else:
        ...
        for i in range(num_table_to_show):
            self.fig.add_subplot(100+num_table_to_show*10+i+1)
        axes=self.fig.get_axes()
        for i in range(0, num_table_to_show):
            the_table = axes[i].table(
                ...
                colLabels=["stock", "data", "action","result","reward","old_prob"],
                loc='center',
            )
            axes[i].axis("off")
