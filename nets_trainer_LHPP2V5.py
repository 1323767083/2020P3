...
def init_nets_trainer_LHPP2V5(lc_in,nc_in):
    global lc, nc
    lc=lc_in
    nc=nc_in
...
class LHPP2V5_PPO_trainer(base_trainer):
    def __init__(self):
    ...
    def build_train_model(self, name="T"):
        ...
        Tmodel = Model(inputs=[input_lv, input_sv, input_a, input_r, input_oldAP],outputs=[con_out], name=name)
    ...
    def optimize_com(self, i_train_buffer, Pmodel, Tmodel):
        ...
        rg=self.get_accumulate_r([n_r, train_sv, l_support_view])
    ...
    def get_accumulate_r(self,inputs):
        n_r, sv, l_support_view_dic = inputs
        l_adjr=[]
        for item_r, item_train_sv, support_view_dic in zip(n_r, sv, l_support_view_dic):
            if support_view_dic[0, 0]["action_return_message"] =="Success" and support_view_dic[0, 0]["action_taken"] == "Buy":
                l_adjr.append(item_r[0])
            elif support_view_dic[0, 0]["action_return_message"] =="No_trans" and support_view_dic[0, 0]["action_taken"] == "No_trans":
                l_adjr.append(-1*(item_r[0] + lc.Brain_gamma**support_view_dic[0, 0]["SdisS_"] * item_train_sv[0]))  # this is because the bigger r is the smaller prob of No_trans
            elif support_view_dic[0, 0]["action_return_message"] =="Tinpai" and support_view_dic[0, 0]["action_taken"] == "Buy":
                l_adjr.append(0.0) #same as no_action
            elif support_view_dic[0, 0]["action_return_message"] =="No_action" and support_view_dic[0, 0]["action_taken"] == "No_action":
                l_adjr.append(0.0)
            else:
    ...
    def M_advent_low(self,y_true, y_pred):
        _, _, _, advent, _= self.extract_y(y_pred)
        advent10=tf.contrib.distributions.percentile(advent, 10., interpolation='lower')
        return advent10

    def M_advent_high(self,y_true, y_pred):
        _, _, _, advent, _= self.extract_y(y_pred)
        advent90=tf.contrib.distributions.percentile(advent, 90., interpolation='higher')
        return advent90
