...
class common_component:
    ...
    def Cov_2D_module(self, kernel, filters,max_pool, input_tensor, name=None):
        if name is not None:
            conv_name,max_pool_name, relu_name= name + '_conv',name + '_pool',name + '_relu'
        else:
            conv_name, max_pool_name, relu_name = None, None, None
        ...
        if max_pool > 1 and a.shape[1]>1 and a.shape[2]>1:  #todo dirty solution for 2D only
            a = keras.layers.MaxPooling2D(pool_size=max_pool, padding='valid', name=max_pool_name)(a)
        d = keras.layers.LeakyReLU(name=relu_name)(a)
        return d
    ...
    def Inception_2D_module(self,filters,input, name=None):
        if name is not None:
            cp, pp =name + "_conv",name + "_pool"
            tower11_name,tower12_name,tower21_name,tower22_name,tower31_name,tower32_name=\
                cp+"_t11",cp+ "_t12",cp+"_t21",cp+"_t22",pp+"_t31",cp+"_t32"
            ...
            tower11_name, tower12_name, tower21_name, tower22_name, tower31_name, tower32_name=\
                None,None,None,None,None,None
            out_name = None
        tower_1 = keras.layers.Conv2D(filters=filters, kernel_size=1, padding='same', activation='relu', name=tower11_name)(input)
        tower_1 = keras.layers.Conv2D(filters=filters, kernel_size=3, padding='same', activation='relu', name=tower12_name )(tower_1)
        tower_2 = keras.layers.Conv2D(filters=filters, kernel_size=1, padding='same', activation='relu', name=tower21_name)(input)
        tower_2 = keras.layers.Conv2D(filters=filters, kernel_size=5, padding='same', activation='relu', name=tower22_name)(tower_2)
        tower_3 = keras.layers.MaxPooling2D(pool_size=3, strides=1, padding='same', name=tower31_name)(input)
        tower_3 = keras.layers.Conv2D(filters=filters, kernel_size=1, padding='same', activation='relu',name=tower32_name)(tower_3)
        output = keras.layers.Concatenate( axis=3, name=out_name)([tower_1, tower_2, tower_3])  # 2d sampel axis =3
        return output
...
class LVSV_component:
    ...
    def CNN2D_get_SV_state(self, input_sv, name):
        ...
        for idx, [kernel, filter, maxpool, flag] in enumerate(zip(self.nc.s_kernel_l, self.nc.s_filter_l,self.nc.s_maxpool_l, self.nc.flag_s_level)):
            ...
            conv_nm, pool_nm, relu_nm = prefix + '_conv', prefix + '_pool', prefix + '_relu'
            if flag == "C":
                immediate_sv = self.cc.Cov_2D_module(kernel, filter, maxpool, immediate_sv, name=conv_nm)
            else:
                assert flag == "I"
                assert kernel == 0
                immediate_sv = self.cc.Inception_2D_module(filter, immediate_sv, name=conv_nm)
                if maxpool > 1 and immediate_sv.shape[1]>1 and immediate_sv.shape[2]>1:  #todo dirty solution for 2D only:
                    immediate_sv = keras.layers.MaxPooling2D(pool_size=maxpool, padding='valid', name=pool_nm)(immediate_sv)
        output_sv = keras.layers.Flatten(name=name + "_sv_flatten")(immediate_sv)
        return output_sv
    ...
    def CNN_get_LV_SV_joint_state(self, inputs, name):
        ...
        lv_sv_joint_state = keras.layers.Reshape((self.nc.l_filter_l[-1],), name=name + self.nc.LNM_LV_SV_joint)(immediate_lv)
    ...
    def CNN2D_get_LV_SV_joint_state(self, inputs, name):
        ...
        for idx, [kernel, filter, maxpool, flag] in enumerate(zip(self.nc.l_kernel_l, self.nc.l_filter_l,
            ...
            conv_name, pool_name = name + "_LVSV_branch{0}".format(idx), name + "_LVSV_pool{0}".format(idx)
            if flag =="C":
                immediate_lv = self.cc.Cov_2D_module(kernel, filter, maxpool, immediate_lv,name=conv_name)
            else:
                assert flag == "I"
                assert kernel == 0
                immediate_lv_t = self.cc.Inception_2D_module(filter, immediate_lv, name=conv_name)
                if maxpool > 1 and immediate_lv_t.shape[1]>1 and immediate_lv_t.shape[2]>1:  #todo dirty solution for 2D only::
                    immediate_lv = keras.layers.MaxPooling2D(pool_size=maxpool, padding='valid', name=pool_name)(immediate_lv_t)

        immediate_lv = keras.layers.Flatten(name=name + "_LVSV_reshaped")(immediate_lv)
        lv_sv_joint_state = keras.layers.Concatenate(axis=1, name=name + self.nc.LNM_LV_SV_joint)([immediate_lv, SV_state])
        return lv_sv_joint_state
