...
from keras.layers import Reshape, Input, Lambda,TimeDistributed
from keras.layers import Concatenate, concatenate  # these two concatenate has difference in usage
...
class common_component:
    def construct_conv_branch(self, cov_kernel_list, cov_filter_list, max_pool_list, input_tensor):
        ...
        for idx, [kernel, filter, maxpool] in enumerate(zip(cov_kernel_list, cov_filter_list, max_pool_list)):
            a = Conv1D(filters=filter, padding='same', kernel_size=kernel)(input_tensor if idx==0 else d)
            b = MaxPooling1D(pool_size=maxpool, padding='valid')(a)
    ...
    def construct_lstms(self,lstm_units_list, input_tensor, name=None):
        ...
        if len(lstm_units_list) == 1:
            ...
            a = LSTM(lstm_units_list[0], name=name_prefix)(input_tensor)
        else:
            for idx, lstm_units in enumerate(lstm_units_list):
                ...
                if idx == len(lstm_units_list) - 1:
                    a = LSTM(lstm_units,name=name_prefix)(a)
                else:
                    a = LSTM(lstm_units, return_sequences=True,name=name_prefix)(input_tensor if idx==0 else a)
        return a
    def construct_denses(self, dense_list, input_tensor, name=None):
        ...
        for idx, dense_number in enumerate(dense_list):
            ...
            a = Dense(dense_number, activation='relu', name=name_prefix)(input_tensor if idx==0 else a)
        return a
    def construct_CuDNNLSTMs(self,lstm_units_list, input_tensor, name=None):
        ...
        if len(lstm_units_list) == 1:
            ...
            a = CuDNNLSTM(lstm_units_list[0], name=name_prefix)(input_tensor)
        else:
            for idx, lstm_units in enumerate(lstm_units_list):
                name_prefix = name + "_CuDNNLSTM{0}".format(idx) if name is not None else None
                if idx == len(lstm_units_list) - 1:
                    a = CuDNNLSTM(lstm_units, name=name_prefix)(a)
                else:
                    a = CuDNNLSTM(lstm_units, return_sequences=True,name=name_prefix)(input_tensor if idx == 0 else a)
        return a
    ...
    def Cov_1D_module(self, kernel, filters,max_pool, input_tensor, name=None):
        ...
        a = Conv1D(filters=filters, padding='same', kernel_size=kernel,name=conv_name)(input_tensor)
        if max_pool>1:
            a = MaxPooling1D(pool_size=max_pool, padding='valid', name=max_pool_name)(a)
        d = LeakyReLU(name=relu_name)(a)
    ...
    def Inception_1D_module(self,filters,input, name=None):
        if name is not None:
            ...
            out_name = None
        tower_1 = Conv1D(filters=filters, kernel_size=1, padding='same', activation='relu', name=tower11_name)(input)
        tower_1 = Conv1D(filters=filters, kernel_size=3, padding='same', activation='relu', name=tower12_name )(tower_1)
        tower_2 = Conv1D(filters=filters, kernel_size=1, padding='same', activation='relu', name=tower21_name)(input)
        tower_2 = Conv1D(filters=filters, kernel_size=5, padding='same', activation='relu', name=tower22_name)(tower_2)
        tower_3 = MaxPooling1D(pool_size=3, strides=1, padding='same', name=tower31_name)(input)
        tower_3 = Conv1D(filters=filters, kernel_size=1, padding='same', activation='relu',name=tower32_name)(tower_3)
        output = Concatenate( axis=2, name=out_name)([tower_1, tower_2, tower_3])  # 2d sampel axis =3
        return output
...
class SV_component:
    def CNN_get_SV_state(self, input, name):
        ...
        for idx, [kernel, filter, maxpool, flag] in enumerate(zip(nc.s_kernel_l, nc.s_filter_l,nc.s_maxpool_l, nc.flag_s_level)):
            ...
            immediate_sv = TimeDistributed(Conv1D(filters=filter, padding='same', kernel_size=kernel, name=conv_nm)
                                           , name="TD_{0}".format(conv_nm))(immediate_sv)
            if maxpool > 1:
                immediate_sv = TimeDistributed(MaxPooling1D(pool_size=maxpool, padding='valid', name=pool_nm)
                                               , name="TD_{0}".format(pool_nm))(immediate_sv)
            immediate_sv = TimeDistributed(LeakyReLU(name=relu_nm), name="TD_{0}".format(relu_nm))(immediate_sv)

        output_sv = Lambda(lambda x: K.squeeze(x, axis=2), name=name + "_sv_squeezed")(immediate_sv)
        return output_sv
    ...
    def RNN_get_SV_state(self, input, name="SV"):
        ...
        for idx, param in enumerate(nc.ms_param_TimeDistributed):
            if idx != last_index:
                immediate_sv = TimeDistributed(CuDNNLSTM(param, return_sequences=True, name="{0}_SV_LSTM_{1}".format(name, idx)),
                                    name="TD_{0}".format(idx))(immediate_sv)
            else:
                immediate_sv = TimeDistributed(CuDNNLSTM(param, name="{0}_SV_LSTM_{1}".format(name, idx)),
                                    name="TD_{0}".format(idx))(immediate_sv)
        return immediate_sv
    ...
    def RCN_get_SV_state(self, input, name):
        ...
        immediate_sv = Concatenate(axis=2, name="{0}_SV_JOINT".format(name))([CNN_immediate_sv, CuDNNLSTM__immediate_sv])
        return immediate_sv
...
class LV_SV_joint_component:
    def _RNN_LV_Sved(self,LV_SVed, name):
        ...
        for idx, param in enumerate(nc.ms_param_CuDNNLSTM):
            if idx != last_index:
                immediate_lv = CuDNNLSTM(param, return_sequences=True, name="{0}_LVSV_LSTM_{1}".format(name, idx))(immediate_lv)
            else:
                immediate_lv = CuDNNLSTM(param, name="{0}_LVSV_LSTM_{1}".format(name, idx))(immediate_lv)
        return immediate_lv
    ...
    def _CNN_LV_SVed(self,LV_SVed, name):
        ...
        for idx, [kernel, filter, maxpool, flag] in enumerate(zip(nc.l_kernel_l, nc.l_filter_l,
            ...
            if flag == "C":
                ...
                immediate_lv = MaxPooling1D(pool_size=maxpool, padding='valid', name=pool_name)(immediate_lv_t)
        immediate_lv = Reshape((nc.l_filter_l[-1],), name=name+"_LVSV_reshaped")(immediate_lv)
        return immediate_lv

    def RNN_get_LV_SV_joint_state(self, inputs, name):
        input_lv, SV_state = inputs
        LV_SVed = Concatenate(axis=2, name=name + "_LV_SVed")([input_lv, SV_state])
        immediate_lv = self._RNN_LV_Sved(LV_SVed, name)
        lv_sv_joint_state= Lambda(lambda  x: x, name=name + LNM_LV_SV_joint)(immediate_lv)
    ...
    def RCN_get_LV_SV_joint_state(self, inputs, name):
        ...
        lv_sv_joint_state = Concatenate(axis=1, name=name + LNM_LV_SV_joint)([immediate_rnn_lv, immediate_cnn_lv])
        return lv_sv_joint_state
